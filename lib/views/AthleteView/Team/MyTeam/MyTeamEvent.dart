...
class MyTeamEvent extends StatefulWidget {
  final String teamId;
  final bool isAdmin;
  final bool isModerator;

  MyTeamEvent({required this.teamId,required this.isModerator, required this.isAdmin});

  @override
  _MyTeamEventState createState() => _MyTeamEventState();
}

class _MyTeamEventState extends State<MyTeamEvent> {
            ...
            child: Row(
              ...
              children: [
                ...
                InkWell(
                  ...
                  onTap: (){
                    Navigator.push(context, MaterialPageRoute(builder: (context)=>AddEventView(creatorId: widget.teamId,userId: userId, type: typeTeam,))).then((value){
                      setState(() {
                        isLoading=true;
                      });
                      getEvents(userId);
  ...
  getEvents(String userId) async {
    ...
    String apiUrl = ApiClient.urlGetTeamEvents;

    print('teamId: '+widget.teamId);
    print('userId: '+userId);

    final response = await http.post(Uri.parse(apiUrl),
        body: {'team_id': widget.teamId,'user_id':userId}).catchError((value) {
      setState(() {
        isLoading = false;
      });
